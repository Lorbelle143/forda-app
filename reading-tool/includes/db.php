<?php
// ── Detect environment ─────────────────────────────────────
$isLive = (strpos($_SERVER['HTTP_HOST'] ?? '', 'infinityfree') !== false
        || strpos($_SERVER['HTTP_HOST'] ?? '', 'aim.page.gd') !== false
        || strpos($_SERVER['HTTP_HOST'] ?? '', 'epizy.com') !== false);

if ($isLive) {
    $host     = 'sql100.infinityfree.com';
    $dbname   = 'if0_41706224_forda_app';
    $username = 'if0_41706224';
    $password = 'Ganzan15';
} else {
    $host     = 'localhost';
    $dbname   = 'forda_app';
...
try {
    ...
    // Only run setup on localhost (XAMPP) — on live, DB is already set up
    if (!$isLive) {
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS reading_materials (
            ...
            reading_number TINYINT UNSIGNED NOT NULL DEFAULT 1,
            model_audio_path VARCHAR(255) DEFAULT NULL,
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS recordings (
            ...
            mispronounced_count TINYINT UNSIGNED DEFAULT NULL,
